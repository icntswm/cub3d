...
/*   Updated: 2022/01/19 12:57:15 by squickfi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	data->floor_cast.ray_dir_x_l = data->player.ray_dir_x - data->player.plane_x;
	data->floor_cast.ray_dir_y_l = data->player.ray_dir_y - data->player.plane_y;
	data->floor_cast.ray_dir_x_l = data->player.ray_dir_x + data->player.plane_x;
	data->floor_cast.ray_dir_y_l = data->player.ray_dir_y + data->player.plane_y;
	data->floor_cast.pos = i - HEIGHT / 2;
	data->floor_cast.pos_z = 0.5 * HEIGHT;
	data->floor_cast.row_distance = data->floor_cast.pos_z / data->floor_cast.pos;
}
...
void	put_horizontal_line(t_data *data, int i)
{
	...
	while (j < WIDTH)
	{
		...
		int color = get_pixel_color(&data->tex.no, i, j);
		my_mlx_pixel_put(&data->tex.picture, j, i, color);
		color = get_pixel_color(&data->tex.so, i, j);
